...
int hash_function2(struct hash_table* hash_table, char* key) {
   ...
   */
	...
	// loop through the string, multiplying each char by increasingly larger nums
	int i = 0;
	for (i = 0; key[i] != '\n'; i++) {
		int j = 0;
		for (j = 0; j < i; j++) {
			key_2 += ( (int) key[i] ) + (j + 8) * (j + 846);
		}
	}
		
  return key_2 % hash_table->size; // mod size to match to index of table
}
...
int hash_table_collisions(struct hash_table* hash_table) {
  ...
  for (i = 0; i < hash_table->size; i++) {
	...
	// descend through each linked list, counting the depth along the way
	if (temp != NULL) {        // skips empty slots in array
	  while (temp != NULL) {   // loops through each linked list
		num_col++;             // increase num_col for each node found
		temp = temp->next;     // move to next node
	  }
 	  num_col--; // corrects for counting the first node in each list
    }
  }
